...
export default function PaymentForm({ defaultValues = {}, includeUser = false, onSubmit, submitLabel = 'Save' }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-900 mb-1">Payment Type</label>
        <select
          name="payment_type"
          value={values.payment_type}
          onChange={handleChange}
          className="w-full border border-gray-300 rounded px-3 py-2 text-sm text-gray-900 focus:ring-2 focus:ring-green-500 focus:border-green-500"
        >
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          {includeUser && values.payment_type === 'one-time' ? (
            <div className="w-full border border-gray-300 rounded px-3 py-2 text-sm text-gray-900 bg-gray-100">₱{(values.amount || 50).toFixed(2)}</div>
          ) : (
            <input
              name="amount"
              value={values.amount}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-900 mb-1">Method</label>
          <select
            name="method"
            value={values.method}
            onChange={handleChange}
      ...
      {values.method === 'cash' && (
        <div>
          <label className="block text-sm font-medium text-gray-900 mb-1">Tendered (₱)</label>
          <input
            name="tendered"
      ...
      {(values.membership_id || values.staff_id) && (
        <div className="grid grid-cols-2 gap-4">
          {values.membership_id && (
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1">Membership ID (optional)</label>
              <input
                name="membership_id"
                value={values.membership_id}
                type="number"
                readOnly
                className="w-full border border-gray-300 rounded px-3 py-2 text-sm text-gray-900 bg-gray-100 cursor-not-allowed"
              />
            </div>
          )}
          {values.staff_id && (
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1">Staff ID</label>
              <input
                name="staff_id"
      ...
      <div className="flex items-center gap-2">
        ...
        <label htmlFor="is_paid" className="text-sm text-gray-900">Mark as paid</label>
      </div>
